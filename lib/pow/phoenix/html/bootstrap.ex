...
unless Pow.dependency_vsn_match?(:phoenix, ">= 1.7.0") do
  defmodule Pow.Phoenix.HTML.Bootstrap do
    ...
    defp input(:text, key) do
      {label(key), ~s(<%= text_input f, #{inspect_key(key)}, class: "form-control" %>),
       error(key)}
    end

    defp input(:password, key) do
      {label(key), ~s(<%= password_input f, #{inspect_key(key)}, class: "form-control" %>),
       error(key)}
    end

    defp inspect_key({:changeset, :pow_user_id_field}),
      do: "Pow.Ecto.Schema.user_id_field(@changeset)"
