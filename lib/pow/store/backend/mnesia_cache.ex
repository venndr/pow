defmodule Pow.Store.Backend.MnesiaCache do
  ...
  @impl GenServer
  def handle_cast(
        {:append_validators, config, records, ttl},
        %{invalidators: invalidators} = state
      ) do
    invalidators =
    ...
    |> case do
      ...
      {_value, expire} ->
        case Kernel.max(expire - timestamp(), 0) do
          ...
          ttl ->
            :mnesia.report_event(
              {:reschedule_invalidator, {@mnesia_cache_tab, key, {:pid, self()}}}
            )
  ...
  defp find_active_cluster_nodes(config) do
    ...
    Enum.filter(db_nodes, fn node ->
      node in visible_nodes and
        :rpc.block_call(node, :mnesia, :system_info, [:is_running]) == :yes
    end)
  end
