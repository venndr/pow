defmodule Pow.Ecto.Schema.ChangesetTest do
  ...
  describe "User.changeset/2" do
    ...
    test "requires user id" do
      ...
      changeset =
        UsernameUser.changeset(%UsernameUser{}, Map.delete(@valid_params_username, "username"))
    ...
    test "can validate with custom e-mail validator" do
      ...
      assert changeset.errors[:email] ==
               {"has invalid format",
                [validation: :email_format, reason: "custom message john.doe@example.com"]}
    ...
    test "uses case insensitive value for user id" do
      ...
      changeset =
        UsernameUser.changeset(%UsernameUser{}, Map.put(@valid_params, "username", "uSerName"))
    ...
    test "validates length of password" do
      ...
      assert changeset.errors[:password] ==
               {"should be at most %{count} character(s)",
                [count: 4096, validation: :length, kind: :max, type: :string]}
    end
    ...
    test "can confirm and hash password" do
      changeset =
        User.changeset(%User{}, Map.put(@valid_params, "password_confirmation", "invalid"))
    ...
    test "handle `confirm_password` conversion" do
      ...
      assert CaptureIO.capture_io(:stderr, fn ->
               ...
               "passing `confirm_password` value to `Pow.Ecto.Schema.Changeset.confirm_password_changeset/3` has been deprecated, please use `password_confirmation` instead"
  ...
  test "validate_email/1" do
    ...
    assert Changeset.validate_email("John..Doe@example.com") ==
    ...
    assert Changeset.validate_email("A@b@c@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email("a\"b(c)d,e:f;g<h>i[j\\k]l@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email("just\"not\"right@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email("this is\"not\\allowed@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email("this\\ still\\\"not\\\\allowed@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email(
             "1234567890123456789012345678901234567890123456789012345678901234+x@example.com"
           ) == {:error, "local-part too long"}

    assert Changeset.validate_email(
             "i_like_underscore@but_its_not_allow_in_this_part.example.com"
           ) == {:error, "invalid characters in dns label"}
    ...
    assert Changeset.validate_email("john.doe@sub-domain-with-hyphen.domain-with-hyphen.com") ==
             :ok
    ...
    assert Changeset.validate_email("john.doe@-example.com") ==
             {:error, "dns label begins with hyphen"}

    assert Changeset.validate_email("john.doe@-example.example.com") ==
             {:error, "dns label begins with hyphen"}

    assert Changeset.validate_email("john.doe@example-.com") ==
             {:error, "dns label ends with hyphen"}

    assert Changeset.validate_email("john.doe@example-.example.com") ==
             {:error, "dns label ends with hyphen"}

    assert Changeset.validate_email("john.doe@invaliddomain$") ==
             {:error, "invalid characters in dns label"}

    assert Changeset.validate_email("john(comment)doe@example.com") ==
             {:error, "invalid characters in local-part"}

    assert Changeset.validate_email("johndoe@example(comment).com") ==
             {:error, "invalid characters in dns label"}
    ...
    assert Changeset.validate_email("john.doe@#{String.duplicate("x", 64)}.com") ==
             {:error, "dns label too long"}

    assert Changeset.validate_email("john.doe@#{String.duplicate("x", 64)}.example.com") ==
             {:error, "dns label too long"}
  end
end
