defmodule Pow.Store.Backend.MnesiaCacheTest do
  ...
  describe "single node" do
    ...
    test "with no `:ttl` config option" do
      assert_raise ConfigError,
                   "`:ttl` configuration option is required for Pow.Store.Backend.MnesiaCache",
                   fn ->
    ...
    test "can match fetch all" do
      ...
      assert MnesiaCache.all(@default_config, ["namespace", :_]) == [
               {["namespace", "key"], "value"}
             ]
    end
    ...
    test "records auto purge with persistent storage" do
      ...
      :mnesia.dirty_write(
        {MnesiaCache, ["pow:test", "key"], {"value", :os.system_time(:millisecond) + 150}}
      )

      # Ignore sync write messages
      flush_process_mailbox()
      # Wait for reschedule event
      assert_receive {:mnesia_system_event, {:mnesia_user, {:reschedule_invalidator, {_, _, _}}}}
      assert MnesiaCache.get(config, "key") == "value"
      # Wait for TTL reached
      assert_receive {:mnesia_table_event, {:delete, _, _}}, 150
      assert MnesiaCache.get(config, "key") == :not_found
    end
    ...
    test "when initiated with unexpected records" do
      ...
      assert CaptureLog.capture_log(
               [format: "[$level]  $message", colors: [enabled: false]],
               fn ->
                 restart(@default_config)
               end
             ) =~
               ~r/\[(warn|warning|)\]  #{Regex.escape("Found an unexpected record in the mnesia cache, please delete it: [\"pow:test\", \"key\"]")}/
    end
    ...
    test "backwards compatible" do
      assert_capture_io_eval(
        ...
        "Pow.Store.Backend.MnesiaCache.put/3 is deprecated. Use `put/2` instead"
      )
      ...
      assert_capture_io_eval(
        ...
        "Pow.Store.Backend.MnesiaCache.keys/1 is deprecated. Use `all/2` instead"
      )
    end
  end
  ...
  describe "distributed nodes" do
    ...
    test "will join cluster" do
      ...
      assert_receive {{:mnesia, ^node_b},
                      {:mnesia_system_event, {:mnesia_user, {:refresh_invalidators, {_, _}}}}}
      ...
      assert_receive {{:mnesia, ^node_b},
                      {:mnesia_table_event,
                       {:delete, {MnesiaCache, [_, "short_ttl_key_set_on_a"]}, _}}},
                     @assertion_timeout

      assert :rpc.call(node_b, MnesiaCache, :get, [config, "short_ttl_key_set_on_a"]) ==
               :not_found
      ...
      assert_receive {{:mnesia, ^node_a},
                      {:mnesia_table_event,
                       {:delete, {MnesiaCache, [_, "short_ttl_key_2_set_on_b"]}, _}}},
                     @assertion_timeout

      assert :rpc.call(node_a, MnesiaCache, :get, [config, "short_ttl_key_2_set_on_b"]) ==
               :not_found
    end
    ...
    test "automaticaly joins cluster with MnesiaCache.Unsplit" do
      ...
      assert_receive {{Logger, node_a}, {:info, "Mnesia cluster initiated on :\"a@127.0.0.1\""}},
                     @assertion_timeout

      assert_receive {{Logger, node_b}, {:info, "Mnesia cluster initiated on :\"b@127.0.0.1\""}},
                     @assertion_timeout

      assert_receive {{Logger, node_c}, {:info, "Mnesia cluster initiated on :\"c@127.0.0.1\""}},
                     @assertion_timeout
      ...
      case {node, type, message} do
        {^node_c, :info,
         "Connection to :\"b@127.0.0.1\" established with no mnesia cluster found for either :\"c@127.0.0.1\" or :\"b@127.0.0.1\""} ->
          :ok

        {^node_b, :info,
         "Connection to :\"c@127.0.0.1\" established with no mnesia cluster found for either :\"b@127.0.0.1\" or :\"c@127.0.0.1\""} ->
          assert_receive {{Logger, ^node_b},
                          {:info,
                           "Skipping reset for :\"b@127.0.0.1\" as :\"c@127.0.0.1\" is the most recent node"}},
                         @assertion_timeout

          assert_receive {{Logger, ^node_c},
                          {:info,
                           "Connection to :\"b@127.0.0.1\" established with no mnesia cluster found for either :\"c@127.0.0.1\" or :\"b@127.0.0.1\""}},
                         @assertion_timeout
      end
      ...
      assert_receive {{Logger, ^node_c},
                      {:warn,
                       "Resetting mnesia on :\"c@127.0.0.1\" and restarting the mnesia cache to connect to :\"b@127.0.0.1\""}},
                     @assertion_timeout

      assert_receive {{Logger, ^node_c}, {:info, "Application mnesia exited: :stopped"}},
                     @assertion_timeout

      assert_receive {{Logger, ^node_c},
                      {:info,
                       "Joined mnesia cluster nodes [:\"b@127.0.0.1\"] for :\"c@127.0.0.1\""}},
                     @assertion_timeout

      assert :rpc.call(node_b, :mnesia, :system_info, [:extra_db_nodes]) == []

      assert Enum.sort(:rpc.call(node_b, :mnesia, :system_info, [:running_db_nodes])) == [
               node_b,
               node_c
             ]
      ...
      assert Enum.sort(:rpc.call(node_c, :mnesia, :system_info, [:running_db_nodes])) == [
               node_b,
               node_c
             ]
      ...
      case {node, type, message} do
        {^node_b, :info,
         "Connection to :\"a@127.0.0.1\" established with :\"b@127.0.0.1\" already being part of a mnesia cluster"} ->
          assert_receive {{Logger, ^node_a},
                          {:info,
                           "Connection to :\"b@127.0.0.1\" established with no mnesia cluster running on :\"a@127.0.0.1\""}},
                         @assertion_timeout
      ...
      assert_receive {{Logger, ^node_a},
                      {:warn,
                       "Resetting mnesia on :\"a@127.0.0.1\" and restarting the mnesia cache to connect to :\"b@127.0.0.1\""}},
                     @assertion_timeout
      ...
      receive do
        {{Logger, ^node_c},
         {:info,
          "Connection to :\"a@127.0.0.1\" established with :\"c@127.0.0.1\" already being part of a mnesia cluster"}} ->
          :ok
      after
        100 -> :ok
      end

      assert_receive {{Logger, ^node_a}, {:info, "Application mnesia exited: :stopped"}},
                     @assertion_timeout
      ...
      case {node, type, message} do
        ...
        {^node_a, :info,
         "Joined mnesia cluster nodes [:\"b@127.0.0.1\", :\"c@127.0.0.1\"] for :\"a@127.0.0.1\""} ->
          :ok
      end

      assert :rpc.call(node_a, :mnesia, :system_info, [:extra_db_nodes]) == [node_b]

      assert Enum.sort(:rpc.call(node_a, :mnesia, :system_info, [:running_db_nodes])) == [
               node_a,
               node_b,
               node_c
             ]
    ...
    test "recovers from netsplit with MnesiaCache.Unsplit" do
      ...
      assert_receive {{:mnesia, ^node_a},
                      {:mnesia_system_event,
                       {:inconsistent_database, :running_partitioned_network, ^node_b}}}

      assert_receive {{Logger, _node},
                      {:info,
                       "The node :\"b@127.0.0.1\" has been healed and joined the mnesia cluster [:\"a@127.0.0.1\"]"}},
                     @assertion_timeout

      assert_receive {{Logger, _node},
                      {:warn,
                       "Detected a netsplit in the mnesia cluster with node " <> _reported_node}},
                     @assertion_timeout
      ...
      assert :rpc.call(node_a, :mnesia, :dirty_read, [{:node_a_table, :key}]) == [
               {:node_a_table, :key, "a"}
             ]

      assert :rpc.call(node_b, :mnesia, :dirty_read, [{:node_b_table, :key}]) == [
               {:node_b_table, :key, "b"}
             ]

      # Shared tables unrelated to Pow can't reconnect
      {:atomic, :ok} =
        :rpc.call(node_a, :mnesia, :create_table, [:shared, [disc_copies: [node_a]]])

      {:atomic, :ok} =
        :rpc.call(node_b, :mnesia, :add_table_copy, [:shared, node_b, :disc_copies])
      ...
      assert_receive {{Logger, _node},
                      {:error,
                       "Can't force reload unexpected tables [:shared] to heal " <> _reported_node}},
                     @assertion_timeout
    ...
    test "when init create cluster fails" do
      ...
      assert {:error,
              {{:create_table,
                {:aborted, {:not_active, Pow.Store.Backend.MnesiaCache, :invalid_node}}},
               _}} =
               :rpc.call(node_a, Supervisor, :start_child, [Pow.Supervisor, {MnesiaCache, config}])

      assert_receive {{Logger, _node},
                      {:error,
                       "Couldn't initialize mnesia cluster because: {:create_table, {:aborted, {:not_active, Pow.Store.Backend.MnesiaCache, :invalid_node}}}"}},
                     @assertion_timeout
    end
    ...
    test "when init join cluster fails" do
      ...
      assert {:error,
              {{:add_table_copy,
                {:aborted, {:no_exists, {Pow.Store.Backend.MnesiaCache, :cstruct}}}},
               _}} =
               :rpc.call(node_b, Supervisor, :start_child, [Pow.Supervisor, {MnesiaCache, config}])

      assert_receive {{Logger, _node},
                      {:error,
                       "Couldn't join mnesia cluster because: {:add_table_copy, {:aborted, {:no_exists, {Pow.Store.Backend.MnesiaCache, :cstruct}}}}"}},
                     @assertion_timeout
    end
    ...
    test "handles `extra_db_nodes: {module, function, arguments}`" do
      ...
      {:ok, _pid} =
        :rpc.call(node_a, Supervisor, :start_child, [
          Pow.Supervisor,
          {MnesiaCache, @default_config}
        ])
  ...
  defp add_listener_module(node) do
    {:module, Pow.Test.Listener, _, _} =
      rpc(node, Module, :create, [
        Pow.Test.Listener,
        quote do
          use GenServer
  ...
  defp start_mnesia_cache(node, config, opts \\ []) do
    ...
    {:ok, _pid} =
      :rpc.call(node, Supervisor, :start_child, [Pow.Supervisor, {MnesiaCache, config}])
    ...
    if opts[:unsplit],
      do:
        {:ok, _pid} =
          :rpc.call(node, Supervisor, :start_child, [Pow.Supervisor, MnesiaCache.Unsplit])
  end
  ...
  defp listen(node, module) when module in [:mnesia, :node] do
    {:ok, _} =
      :rpc.call(node, Supervisor, :start_child, [
        Pow.Supervisor,
        {Pow.Test.Listener, {module, self()}}
      ])
  end
  ...
  defp reset_unsplit_trigger_inconsistent_database(node, cluster_node, config) do
    :ok =
      :rpc.block_call(node, Supervisor, :terminate_child, [Pow.Supervisor, MnesiaCache.Unsplit])

    :ok = :rpc.block_call(node, Supervisor, :delete_child, [Pow.Supervisor, MnesiaCache.Unsplit])

    {:ok, pid} =
      :rpc.block_call(node, Supervisor, :start_child, [
        Pow.Supervisor,
        {MnesiaCache.Unsplit, config}
      ])

    :rpc.call(node, Kernel, :send, [
      pid,
      {:mnesia_system_event, {:inconsistent_database, nil, cluster_node}}
    ])
  end
  ...
  describe "backwards compatible" do
    ...
    test "removes old entries" do
      ...
      :ok =
        :mnesia.dirty_write(
          {MnesiaCache, key,
           {"key1", "test", @default_config, :os.system_time(:millisecond) + 100}}
        )
      ...
      assert CaptureLog.capture_log(
               ...
               ~r/\[(warn|warning|)\]  #{Regex.escape("Deleting old record in the mnesia cache: \"pow:test:key1\"")}/
