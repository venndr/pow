defmodule PowPersistentSession.Plug.CookieTest do
  ...
  test "call/2 assigns user from cookie and doesn't expire with simultanous request", %{
    conn: conn,
    user: user
  } do
    ...
    config = [
      persistent_session_store:
        {PersistentSessionCacheWaitDelete,
         ttl: :timer.hours(24) * 30, namespace: "persistent_session"}
    ]
  ...
  test "call/2 assigns user from cookie passing fingerprint to the session metadata", %{
    conn: conn,
    user: user
  } do
  ...
  test "call/2 assigns user from cookie passing custom metadata to session metadata", %{
    conn: conn,
    user: user
  } do
    id =
      store_in_cache(
        conn,
        "test",
        {user, session_metadata: [a: 1, b: 2, fingerprint: "fingerprint"]}
      )
    ...
    assert get_from_cache(conn, id) ==
             {user, session_metadata: [fingerprint: "new_fingerprint", b: 2, a: 2]}

    assert [inserted_at: _, b: 2, a: 3, fingerprint: "new_fingerprint"] =
             conn.private[:pow_session_metadata]
  end
  ...
  test "call/2 assigns user from cookie with prepended `:otp_app`", %{config: config, user: user} do
    ...
    assert %{value: new_id, max_age: @max_age, path: "/"} =
             conn.resp_cookies["test_app_" <> @cookie_key]
  ...
  test "call/2 handles clause error", %{conn: conn} do
    ...
    assert_raise RuntimeError,
                 "Primary key value for key `:id` in #{inspect(User)} can't be `nil`",
                 fn ->
                   conn
                   |> persistent_cookie(@cookie_key, id)
                   |> run_plug()
  ...
  test "call/2 is backwards-compatible with `:session_fingerprint` metadata", %{
    conn: conn,
    user: user
  } do
  ...
  test "create/3 with `[:pow_session_metadata][:fingerprint]` defined in conn.private", %{
    conn: conn,
    config: config,
    user: user
  } do
    ...
    assert_received {:ets, :put,
                     [{_key, {^user, session_metadata: [fingerprint: "fingerprint"]}}], _config}
  end
